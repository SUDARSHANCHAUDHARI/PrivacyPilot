# Plugin 3 — PrivacyPilot
**Claude Code Plugin | Public — for any Android developer**
...
Reads an Android app's `AndroidManifest.xml` and Gradle dependencies, automatically detects all permissions and third-party SDKs (AdMob, Firebase, Crashlytics, etc.), and generates a fully compliant privacy policy HTML page. Deploys to any GitHub Pages URL.

**Public plugin — works for any Android developer worldwide.** Run `/privacypilot:setup` once per project. All commands use your saved config automatically.
...
1. Check if .claude-plugin-config.json already exists:
...
4. Add .claude-plugin-config.json to .gitignore (create if missing)
...
### Config loading rule (applied to ALL other commands)

Every command checks for config before doing anything:
- Found → load silently, proceed
- Not found → stop: "Run /privacypilot:setup first."
- Use `{config.developer.name}`, `{config.developer.email}`, `{config.developer.company}`, `{config.github_pages.privacy_policy_pattern}` etc. — never hardcode
...
1. Ask for:
   ...
   - Package name (e.g. {config.android.package_prefix}.myapp)
...
6. Output summary:
   ...
   Privacy policy URL: https://{config.github_pages.privacy_policy_pattern}
...
2. Show the deployment plan:
   ...
   GitHub Pages URL: https://{config.github_pages.privacy_policy_pattern}
   Branch: main
   File: index.html
   ```
...
description: Deploying privacy policy HTML to GitHub Pages for {config.developer.company} apps
trigger: when deploying or updating a privacy policy to GitHub Pages
---

## GitHub Pages Deployment Pattern

### URL convention ({config.developer.company})
`https://{config.github_pages.privacy_policy_pattern}`

Examples:
- YourApp → `https://sudarshanchaudhari.github.io/yourapp-privacy-policy/`
- YourApp2 → `https://sudarshanchaudhari.github.io/yourapp2-privacy-policy/`
...
Enter: `https://{config.github_pages.privacy_policy_pattern}`
```
...
<p>We retain collected information for as long as necessary to provide our services. Crash reports and analytics data are retained for up to 90 days. If you wish to request deletion of your data, please contact us at {config.developer.email}.</p>
...
<p>Our App is not directed to children under the age of 13. We do not knowingly collect personally identifiable information from children under 13. If you are a parent or guardian and believe your child has provided us with personal information, please contact us at {config.developer.email}.</p>
...
<p>To exercise these rights, contact us at {config.developer.email}.</p>
...
<ul>
  <li>Email: {config.developer.email}</li>
  <li>Developer: {config.developer.name}</li>
  <li>Country: {config.developer.country}</li>
</ul>
...
- [ ] Test `generate` command on your app (e.g. com.yourcompany.yourapp)
